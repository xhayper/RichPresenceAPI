...
namespace Lachee.IO
{
    public class NamedPipeClientStream : Stream
    {
        ...
        /// <summary>
        ///     Reads a block of bytes from a stream and writes the data to a specified buffer. Will not block if there is no data
        ///     available to read.
        /// </summary>
        /// <param name="buffer">
        ...
        /// <returns>
        ///     The total number of bytes that are read into buffer. This might be less than the number of bytes requested if
        ///     that number of bytes is not currently available. If the value is less than 0, then a error has occured.
        /// </returns>
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            var buffptr = Marshal.AllocHGlobal(size);
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            try
            {
                ...
                //Send the block
                var result = Native.WriteFrame(ptr, buffptr, count);
        ...
        private static class Native
        {
            ...
            [DllImport(LIBRARY_NAME, EntryPoint = "isConnected", CharSet = CharSet.Ansi,
                CallingConvention = CallingConvention.Cdecl)]
            public static extern bool IsConnected([MarshalAs(UnmanagedType.SysInt)] IntPtr client);

            [DllImport(LIBRARY_NAME, EntryPoint = "open", CharSet = CharSet.Ansi,
                CallingConvention = CallingConvention.Cdecl)]
            public static extern int Open(IntPtr client, [MarshalAs(UnmanagedType.LPStr)] string pipename);

            [DllImport(LIBRARY_NAME, EntryPoint = "close", CharSet = CharSet.Ansi,
                CallingConvention = CallingConvention.Cdecl)]
        ...
        public void Connect()
        {
            var code = Native.Open(ptr, PipeName);
